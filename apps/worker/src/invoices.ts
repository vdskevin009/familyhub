...
export async function classify(mail: Mail, email: string, label = email, diagnostic = false): Promise<{ result: Classification; source: Invoice["ClassificationSource"] }> {
  ...
  if (rule) {
    ...
    return { source: "rules", result: { kind: rule.kind, confidence, transaction: proof.transaction,
      ...
      serviceDate: null, billedAmount: null, reimbursedAmount: null, reason: confidence >= .9 ? "Décision répétée appliquée automatiquement." : "Décision précédente proposée; confirmez-la encore pour augmenter l'autonomie." } };
  }
  ...
  if (metadataRule) return { source: "rules", result: { kind: metadataRule.kind, confidence: .99, transaction: false,
    ...
    serviceDate: null, billedAmount: null, reimbursedAmount: null, reason: metadataRule.reason } };
  if (proof.marketing) return { source: "rules", result: { kind: "marketing", confidence: .98, transaction: false,
    ...
    serviceDate: null, billedAmount: null, reimbursedAmount: null, reason: "Promotional signals without evidence of a completed transaction." } };
  try {
    ...
    const prompt = [
      ...
      JSON.stringify({ accountLabel: label, subject: mail.subject, sender: mail.sender, text: mail.text, attachmentNames: mail.attachments.map(x => x.FileName) })
    ].join("\n");
    ...
    return { source: "unavailable", result: { kind: "other", confidence: 0, transaction: false, reimbursement: "unknown",
      ...
      billedAmount: null, reimbursedAmount: null, reason: "Classification unavailable; review this email manually. The collector will retry." } };
  }
}
    ...
    for (const account of accounts) {
      ...
      try {
        ...
        const processMessage = async (id: string, retry = false) => {
          ...
          const needsUpgrade = existing && (!existing.DocumentRole || !existing.Member || !("BilledAmount" in existing));
        ...
        for (const item of [...state.items]) {
          if (item.AccountEmail === key && (!item.DocumentRole || !item.Member || !("BilledAmount" in item)) && upgradeBudget-- > 0) await processMessage(item.SourceMessageId, true);
        }
        for (const item of [...state.items]) {
        ...
        for (let pages = 0; pages < 2; pages++) {
          const q = `after:${window.after} before:${window.before} {receipt invoice facture reçu recu reimbursement remboursement claim statement "payment confirmation" "amount due" "booking confirmation" "reservation confirmation" "explanation of benefits" "renewal notice" "blue cross" "croix bleue" desjardins} -in:spam -in:trash -in:sent -in:drafts -from:notifications@github.com`;
...
export async function correctInvoice(id: string, kind: unknown): Promise<Invoice> {
  ...
  await edit(() => {
    ...
    state.corrections.push({ account: result.AccountEmail, fingerprint: result.Fingerprint, kind: result.DocumentType, at: result.CorrectedAt!, confirmations: (previous?.confirmations ?? 0) + 1 });
  });
  return result!;
}
