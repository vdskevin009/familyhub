...
export function assistantInsights(
  ...
  const claims = reimbursements.Items.filter(item => item.Status === ReimbursementStatus.ToReview || item.Status === ReimbursementStatus.ReadyToClaim);
...
export function buildAssistantContext(
  ...
  return JSON.stringify({
    ...
    reimbursementQueue: reimbursements.Items.filter(item => item.Status <= ReimbursementStatus.ReadyToClaim).map(item => ({
